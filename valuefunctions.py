...
class ValueFunctionDQN:
    def __init__(self, scope="MyValueFunctionEstimator", state_dim=2, n_actions=3, train_batch_size=64,
                 ...
                 restoration_checkpoint=None, n_embeddings=0, epsilon0=0.0, summarize_internal_excitations=False, target_scale=1):
        # Input check
    ...
    def train(self, states, targets, w=None, summaries_to_save=None):
        ...
        feed_dict = {self.x: states, self.train_targets: targets/self.target_scale}
        if self.apply_wis:
...
class ValueFunctionDuelingDQN:
        ...
        with tf.variable_scope(self.scope):
            ...
            for l in range(len(self.layers_size) - 2):
                ...
                self.trained_b.append(tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES,
                                                        scope=self.scope + "/b" + str(l)))
            l += 1
            self.weights.append(tf.get_variable(name="w_val" + str(l), shape=[self.layers_size[l],
                                                                          1],
                                                initializer=tf.contrib.layers.xavier_initializer()))
            self.biases.append(tf.get_variable(name="b_val" + str(l), shape=[1],
                                               initializer=tf.constant_initializer(0.0)))

            self.weights.append(tf.get_variable(name="w_adv" + str(l), shape=[self.layers_size[l],
                                                                          self.layers_size[l + 1]],
                                                initializer=tf.contrib.layers.xavier_initializer()))
            self.biases.append(tf.get_variable(name="b_adv" + str(l), shape=[self.layers_size[l + 1]],
                                               initializer=tf.constant_initializer(0.0)))

            self.weights_old.append(tf.get_variable(name="w_val-" + str(l),
                                                    initializer=self.weights[l].initialized_value()))
            self.biases_old.append(tf.get_variable(name="b_val-" + str(l),
                                                   initializer=self.biases[l].initialized_value()))
            self.weights_old.append(tf.get_variable(name="w_adv-" + str(l),
                                                    initializer=self.weights[l+1].initialized_value()))
            self.biases_old.append(tf.get_variable(name="b_adv-" + str(l),
                                                   initializer=self.biases[l+1].initialized_value()))
    ...
    def __model(self, x, use_old_params=False):
        ...
        for l in range(len(self.layers_size)-2):
            ...
            hidden.append(tf.nn.relu(z[l], name="hidden_" + str(l + 1)))
        if use_old_params:
            value = tf.matmul(hidden[-1], self.weights_old[-2]) + self.biases_old[-2]
            advantage = tf.matmul(hidden[-1], self.weights_old[-1]) + self.biases_old[-1]
            z.append(value + advantage - tf.reduce_mean(advantage))
        else:
            value = tf.matmul(hidden[-1], self.weights[-2]) + self.biases[-2]
            advantage = tf.matmul(hidden[-1], self.weights[-1]) + self.biases[-1]
            z.append(value + advantage - tf.reduce_mean(advantage))
